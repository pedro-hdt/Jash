...
public class WcApplication implements WcInterface {
    ...
    public String countFromFiles(Boolean isBytes, Boolean isLines, Boolean isWords, //NOPMD
        ...
        if (fileName.length == 0) {
            throw new WcException(ERR_MISSING_ARG);
        }
        ...
        for (String file : fileName) {
            ...
            if (!node.exists()) {
                result.add("wc: " + node.getName() + ": " + ERR_FILE_NOT_FOUND);
                continue;
            }
            if (node.isDirectory()) {
                result.add("wc: " + node.getName() + ": " + ERR_IS_DIR);
                continue;
            }
            if (!node.canRead()) {
                result.add("wc: " + node.getName() + ": " + ERR_NO_PERM);
                continue;
            }
    ...
    public String countFromStdin(Boolean isBytes, Boolean isLines, Boolean isWords,
        ...
        if (isBytes == null || isLines == null || isWords == null) {
            throw new WcException(ERR_NULL_ARGS);
        }
