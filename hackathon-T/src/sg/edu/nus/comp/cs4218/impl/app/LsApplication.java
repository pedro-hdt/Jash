...
public class LsApplication implements LsInterface {
    ...
    private String buildResult(List<Path> paths, Boolean isFoldersOnly, Boolean isRecursive) {
        ...
        for (Path path : paths) {
            try {
                ...
                String relativePath = PATH_CURR_DIR + getRelativeToCwd(path).toString();
    ...
    /**
     * Pad result with two new lines if not already present.
     *
     * @param result - final result
     * @return
     */
    private void padResult(StringBuilder result) {
        if (result.length() <= 4) {
            return;
        }

        if (!result.substring(result.length() - 1, result.length()).equals(StringUtils.STRING_NEWLINE)) {
            result.append(StringUtils.STRING_NEWLINE);
        }

        if (!result.substring(result.length() - 2, result.length() - 1).equals(StringUtils.STRING_NEWLINE)) {
            result.append(StringUtils.STRING_NEWLINE);
        }
    ...
    private List<Path> getContents(Path directory, Boolean isFoldersOnly)
            throws InvalidDirectoryException, NotDirectoryException {
        if (!Files.exists(directory)) {
            throw new InvalidDirectoryException(getRelativeToCwd(directory).toString());
        }

        if (!Files.isDirectory(directory)) {
            throw new NotDirectoryException(getRelativeToCwd(directory).toString());
        }
    ...
    private Path resolvePath(String directory) {
        ...
        return Paths.get(environmentDir, directory).normalize();
    }
    ...
    private Path getRelativeToCwd(Path path) {
        return Paths.get(environmentDir).relativize(path);
    }

    public void initialiseForTest() {
        environmentDir = Environment.LS_TEST_DIRECTORY;
    }
